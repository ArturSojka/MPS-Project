...
class ShowcaseScene3v2(Scene):
    def construct(self):
        ...
        gr1 = [RED_E,RED_A]
        sim.simulate(gen1, 10, 500, 1e-5)
        paths1 = m_sim.as_animated_paths(gr1)
        objects1, animations1 = m_sim.as_at_once(gr1,0.1)
        
        gr2 = [BLUE_E,BLUE_A]
        sim.simulate(gen2, 10, 500, 1e-5)
        paths2 = m_sim.as_animated_paths(gr2)
        objects2, animations2 = m_sim.as_at_once(gr2,0.1)

        self.add(m_gen1.as_rect())
        self.add(m_gen2.as_rect())
        self.add(*objects1,*objects2)
        self.play(*paths1,*paths2, *animations1, *animations2, rate_func=linear, run_time=10)
        self.wait()

class ShowcaseScene4(Scene):
    def construct(self):
        field = ElectricField(10, 0.1)
        conv = CoordsConverter(
            field.nx, field.ny,
        ...
        gr1 = [RED_E,RED_A]
        sim.simulate(gen1, 10, 1000, 1e-5)
        objects1, animations1 = m_sim.as_at_once(gr1,0.1)

        gr2 = [BLUE_E,BLUE_A]
        sim.simulate(gen2, 10, 1000, 1e-5)
        objects2, animations2 = m_sim.as_at_once(gr2,0.1)

        self.add(m_gen1.as_rect())
        self.add(m_gen2.as_rect())
        self.add(*objects1,*objects2)
        self.play(*animations1, *animations2, rate_func=linear, run_time=10)
        self.wait()

class ShowcaseScene5(Scene):
    def construct(self):
        field = ElectricField(10, 0.1)
        conv = CoordsConverter(
            field.nx, field.ny,
        ...
        for i in (0, 2, 4, 6, 8):
            c = Conductor(1, (7.5,8.5), (i, i+1))
            field.add(c)
        
        for i in (0, 2, 4, 6, 8):
            c = Conductor(-1, (11.5,12.5), (i, i+1))
            field.add(c)
...
class ShowcaseScene6(Scene):
    def construct(self):
        field = ElectricField(20, 0.1)
        conv = CoordsConverter(
            field.nx, field.ny,
            field.nx*field.dx, field.ny*field.dy,
            field.dx, config["frame_width"], config["frame_height"]
        ...
        for i in (0, 4, 8, 12, 16):
            c = Conductor(100, (10,11), (i+0.5, i+1.5))
            field.add(c)
        
        for i in (0, 4, 8, 12, 16):
            c = Conductor(-50, (15,16), (i+0.5, i+1.5))
            field.add(c)
        ...
        gen1 = IonGenerator(PROTON_MASS, PROTON_CHARGE, (200000, 0), (1.0, 2.0), (3.0, 15.0))
        m_gen1 = MIonGenerator(gen1, conv)
        ...
        gr1 = [RED_E,RED_A]
        sim.simulate(gen1, 10, 150, 1e-5)
        objects1, animations1 = m_sim.as_at_once(gr1,0.1)

        self.add(m_gen1.as_rect())
        self.add(*objects1)
        self.play(*animations1, rate_func=linear, run_time=2)
        self.wait()